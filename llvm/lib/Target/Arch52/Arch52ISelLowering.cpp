...
#include "Arch52.h"
#include "Arch52MachineFunctionInfo.h"
...
static const MCPhysReg ArgGPRs[] = {Arch52::R9, Arch52::R10, Arch52::R11,
                                    Arch52::R12};

void Arch52TargetLowering::ReplaceNodeResults(SDNode *N,
...
#include "Arch52GenCallingConv.inc"
...
SDValue
Arch52TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_Arch52);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, Arch52::R1, PtrVT);
      SDValue Address =
  ...
  Chain = DAG.getNode(Arch52ISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_Arch52);
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
                                   ...
                                   const Arch52Subtarget &Subtarget) {
...
static SDValue unpackFromRegLoc(SelectionDAG &DAG, SDValue Chain,
                                const CCValAssign &VA, const SDLoc &DL,
                                const Arch52TargetLowering &TLI) {
  MachineFunction &MF = DAG.getMachineFunction();
...
/// Arch52 formal arguments implementation
SDValue Arch52TargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Arch52);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &Arch52::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    Arch52FunctionInfo *UFI = MF.getInfo<Arch52FunctionInfo>();
...
bool Arch52TargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  ARCH52_DUMP_RED
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, RVLocs, Context);
  if (!CCInfo.CheckReturn(Outs, RetCC_Arch52))
    return false;
...
SDValue
Arch52TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool IsVarArg,
  ...
  const Arch52Subtarget &STI = MF.getSubtarget<Arch52Subtarget>();
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Arch52);
  ...
  return DAG.getNode(Arch52ISD::RET, DL, MVT::Other, RetOps);
}
...
SDValue Arch52TargetLowering::PerformDAGCombine(SDNode *N,
...
bool Arch52TargetLowering::isLegalAddressingMode(const DataLayout &DL,
